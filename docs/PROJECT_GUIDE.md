...
### 4.7 Streamable HTTP 传输

Server 还提供 Streamable HTTP 入口。`mcp-todo-http` 默认选择 HTTP；通用入口也可以
通过 `--transport streamable-http` 显式选择：

```bash
uv run mcp-todo-http --host 127.0.0.1 --port 8000 --db ./data/todos.db
```
...
`parse_args()` 解析 transport、host、port 和数据库路径；`run_server()` 将 HTTP
设置写入 FastMCP 后启动对应传输。端口必须位于 1 到 65535 之间。stdio 仍然是
`mcp-todo-server` 的默认值，因此已有 MCP Host 配置不受影响。

与 stdio 不同，HTTP Server 由用户或服务管理器常驻启动，MCP Host 只负责连接 URL。
这更适合 Windows LM Studio 访问 WSL 服务，也更容易供多个 Client 复用。
...
`pyproject.toml` 定义了三个命令：

```toml
[project.scripts]
mcp-todo-server = "mcp_todo.server:main"
mcp-todo-http = "mcp_todo.server:http_main"
...
`tests/test_server_cli.py` 验证 stdio 默认值、HTTP 便捷入口、host/port/database 参数，
以及非法端口的拒绝逻辑。实际 HTTP 传输还会使用官方 MCP HTTP Client 做端到端
冒烟验证。
...
- Streamable HTTP Client/Server 通信；
- Codex MCP 集成；
...
- HTTP 模式尚未增加认证和 TLS，仅适合本机或可信开发网络；
